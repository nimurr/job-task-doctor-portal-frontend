...
export default function DoctorsPage() {
  ...
  const submit = async (data: Pick<Doctor, "name" | "specialization" | "hospital" | "phone" | "email">) => { await dispatch(saveDoctor({ id: editing?.id, data })).unwrap(); setOpen(false); setEditing(null); dispatch(fetchDoctors({ search, page, limit: 10 })); };
  const remove = async (id: string) => { if (window.confirm("Delete this doctor? Doctors with patients cannot be deleted.")) { await dispatch(removeDoctor(id)).unwrap(); dispatch(fetchDoctors({ search, page, limit: 10 })); } };
